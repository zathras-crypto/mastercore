...
//    string shortTXID = QString::fromStdString(index.data(Qt::DisplayRole).toString().toStdString().substr(0,12));
    QString txidsender = index.data(Qt::UserRole + 5).toString() + "   (" + QString::fromStdString(index.data(Qt::DisplayRole).toString().toStdString().substr(0,16)) + "...)";
//    txidstatus += "....\tSTATUS: " + index.data(Qt::UserRole + 4).toString();
    ...
    if (!ic.isNull())
    {
        r = option.rect.adjusted(100, 10, -10, -10);
    ...
    r = option.rect.adjusted(5, 0, -10, -25);
    painter->drawText(r.left(), r.top(), r.width(), r.height(), Qt::AlignBottom|Qt::AlignLeft, status, &r);
    // add the datetime
    font.setItalic(false);
    painter->setFont(font);
    r = option.rect.adjusted(5, 25, -10, 0);
    painter->drawText(r.left(), r.top(), r.width(), r.height(), Qt::AlignLeft, "01/01 12:12", &r);
    // add the displaytext
    painter->setPen(penBlack);
    r = option.rect.adjusted(imageSpace, 0, -10, -25);
    font.setBold(true);
    painter->setFont(font);
    ...
    font.setBold(false);
    painter->setFont(font);
    r = option.rect.adjusted(imageSpace, 25, -10, 0);
