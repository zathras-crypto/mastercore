...
static CTransaction TxClassD(const std::vector<CTxOut>& txInputs)
{
    ...
    BOOST_CHECK(OmniCore_Encode_ClassD(vchPayload, txOutputs));
...
static bool GetFirstSender(const std::vector<CTxOut>& txInputs, std::string& strSender)
{
    ...
    CTransaction dummyTx = TxClassD(txInputs);
...
BOOST_AUTO_TEST_CASE(first_vin_is_sender)
{
    ...
    vouts.push_back(createTxOut(100, "LcMzcfw4qFASns8QuutTSMYuWCWaFki118")); // Winner
    vouts.push_back(createTxOut(999, "3KYen723gbjhJU69j8jRhZU6fDw8iVVWKy"));
    vouts.push_back(createTxOut(200, "LZV47BSn8qpZk2RBGJxp6utpF2GXLuRNCt"));

    std::string strExpected("LcMzcfw4qFASns8QuutTSMYuWCWaFki118");
