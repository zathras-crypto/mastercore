...
Value gettradehistoryforpair_OMNI(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            "gettradehistory_MP propertyid propertyid ( count )\n"
            ...
            "3. count                (number, optional) number of trades to retrieve (default: 100)\n"
            "\nResult:\n"
            "[                             (array of MetaDEx trades)\n"
            "  ...\n"
            "]\n"
    ...
    uint64_t count = (params.size() > 2) ? params[2].get_uint64() : 100;
